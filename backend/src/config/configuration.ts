export default () => {
  const config = {
    ...
    branding: {
      companyName: process.env.BRAND_COMPANY_NAME || process.env.APP_NAME || 'Task Manager',
    ...
    smtp: {
      ...
      fromName: process.env.SMTP_FROM_NAME || process.env.BRAND_COMPANY_NAME || 'Task Manager',
